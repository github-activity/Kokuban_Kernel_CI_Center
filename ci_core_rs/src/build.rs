...
/// Cross-build the OpenZFS userland statically and drop it next to the modules.
///
/// Delegates to scripts/build_zfs_userland.sh, which needs a musl cross
/// toolchain and four cross-built dependencies, and is far more legible as a
/// shell script than as generated commands.
fn build_zfs_userland(zfs: &ZfsConfig, jobs: &str, anykernel_dir: &Path) -> Result<()> {
    let script = env::current_dir()?.join("scripts/build_zfs_userland.sh");
    if !script.exists() {
        return Err(anyhow!("Userland build script not found at {:?}", script));
    }

    let out_dir = anykernel_dir.join("zfs/bin");
    fs::create_dir_all(&out_dir)?;

    println!("Building static OpenZFS userland (this adds several minutes)...");
    run_cmd(
        &[
            "bash",
            script
            ...
            zfs_tag(zfs),
            jobs.trim_start_matches("-j"),
            out_dir
                .to_str()
                .ok_or_else(|| anyhow!("Invalid userland output path"))?,
        ],
        None,
        false,
    )?;
...
fn package_zfs_modules(anykernel_dir: &Path, spl_ko: &Path, zfs_ko: &Path) -> Result<()> {
    ...
    fs::write(
        ...
        "OpenZFS kernel modules for this build.\n\
         ...
         \n\
         If a bin/ directory is present next to these modules it holds a\n\
         statically linked userland (zpool, zfs, zdb, ...) built against musl.\n\
         Those need no shared libraries, so they run from anywhere:\n\
         \n\
             adb push bin/* /data/adb/zfs/bin/\n\
             adb shell su -c 'chmod 755 /data/adb/zfs/bin/*'\n\
             adb shell su -c '/data/adb/zfs/bin/zpool version'\n\
         \n\
         They default to /data/adb/zfs/etc for zpool.cache and hostid, since\n\
         /etc is read-only on Android.\n",
    )?;
    Ok(())
}
...
pub fn handle_build(
    ...
    if let Some(ref zfs) = proj.zfs {
        ...
        if zfs.userland.unwrap_or(false) {
            build_zfs_userland(zfs, &jobs, Path::new("AnyKernel3"))?;
            println!("Packaged static userland into AnyKernel3/zfs/bin/");
        }
    }
